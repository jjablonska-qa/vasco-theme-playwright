...
const BASE_URL = 'https://vasco-translator.be/nl/';
const ALL_PRODUCTS_PATH = '/nl/alle-producten/';
const CHECKOUT_PATH = '/bestelling';
const CART_PATH = '/nl/winkelwagen?action=show';
const PRODUCT_Q1 = { id: '38' };

const SHIPPING_METHOD_LABEL = 'UPS Standard';
const COD_PAYMENT_LABEL = 'Betalen onder rembours';

const BE_NL_CUSTOMER = {
  firstName: 'Automat',
  lastName: 'Test',
  email: 'testcases.web@gmail.com',
  phone: '0470123456',
  address1: 'Nieuwstraat 123',
  address2: '2',
  postcode: '1000',
  city: 'Brussel',
  country: 'België',
...
for (const scenario of scenarios) {
  test(`BE NL COD dla ${scenario.quantity}x Q1 jest ${scenario.expectedCodVisible ? 'widoczne' : 'ukryte'} - ${scenario.label}`, async ({ page, browserName }) => {
    await skipWhenStoreUnavailable(page, BASE_URL, 'BE NL store is currently unreachable from the test environment.');
    await seedCartWithQ1Quantity(page, scenario.quantity);
    await goToCheckout(page);
    await completePersonalInformation(page, browserName);
    await completeAddress(page, BE_NL_CUSTOMER);
...
async function goToCheckout(page) {
  ...
  await expect(page).toHaveURL(/\/bestelling/i);
  await expect(page.getByText(/Persoonlijke gegevens|Persoonlijke informatie/i).first()).toBeVisible({ timeout: 20000 });
}

async function completePersonalInformation(page, browserName) {
  await fillField(page, [/^Voornaam/i], BE_NL_CUSTOMER.firstName);
  await fillField(page, [/^Achternaam|^Naam/i], BE_NL_CUSTOMER.lastName);
  await fillField(page, [/E-mail|Email/i], createUniqueEmail(BE_NL_CUSTOMER.email));

  for (const label of [/Servicevoorwaarden|algemene voorwaarden/i, /Privacybeleid|privacyverklaring|privacy/i]) {
...
async function continueToAddressStep(page, browserName) {
  ...
  const addressStepTargets = [
    page.getByText(/^Adressen$|^Adres$/i).first(),
    page.locator('[role="tab"]').filter({ hasText: /^Adressen$|^Adres$/i }).first(),
    page.locator('[role="button"]').filter({ hasText: /^Adressen$|^Adres$/i }).first(),
  ];
...
async function completeAddress(page, customer) {
  ...
  await fillByPlaceholder(page, /voornaam/i, customer.firstName);
  await fillByPlaceholder(page, /achternaam|naam/i, customer.lastName);
  await fillByPlaceholder(page, /^enter adres$|^invoeren adres$|^adres$/i, customer.address1);
  await fillByPlaceholder(page, /toevoeging adres|adresregel 2|aanvulling/i, customer.address2).catch(() => {});
  await fillByPlaceholder(page, /postcode/i, customer.postcode);
  await fillByPlaceholder(page, /plaats|stad/i, customer.city);
  await fillPhoneNumber(page, customer.phone);

  const continueButton = page.getByRole('button', { name: /Verder|Doorgaan/i }).last();
  await expect(continueButton).toBeVisible({ timeout: 15000 });
...
async function waitForShippingAndPaymentStep(page) {
  await expect
    .poll(async () => {
      await closeBlockingPopups(page);
      const loadingVisible = await hasVisibleText(page, /We controleren uw gegevens|Gegevens worden gecontroleerd/i);
      if (loadingVisible) return false;

      const shippingReady = await hasVisibleText(page, /Verzendingswijze|Verzendmethode|Verzending/i);
      const paymentReady = await hasVisibleText(page, /Hoe wil je betalen\?|Hoe wilt u betalen\?|Betalingsmethode/i);
...
async function continueToPaymentStep(page) {
  ...
  await expect
    .poll(async () => {
      const bodyText = await page.locator('body').innerText().catch(() => '');
      return /Hoe wil je betalen\?|Hoe wilt u betalen\?|Betalingsmethode/i.test(bodyText);
    }, { timeout: 30000 })
    .toBeTruthy();
}
...
async function openAllProducts(page) {
  ...
  const mainMenuLink = page.getByRole('link', { name: /Alle producten/i }).first();
  const visibleAllProductsLink = page.locator(`a[href*="${ALL_PRODUCTS_PATH}"]`).first();
  const shopMenuItem = page.getByRole('menuitem', { name: /winkel|vertalers|producten/i }).first();

  if (await desktopMenuLink.isVisible().catch(() => false)) {
    await clickAndWaitForUrl(page, desktopMenuLink, /\/nl\/alle-producten\/?$/i);
  } else {
    ...
    if (await mainMenuLink.isVisible().catch(() => false)) {
      await clickAndWaitForUrl(page, mainMenuLink, /\/nl\/alle-producten\/?$/i);
    }

    if (!/\/nl\/alle-producten\/?$/i.test(page.url()) && (await visibleAllProductsLink.isVisible().catch(() => false))) {
      await clickAndWaitForUrl(page, visibleAllProductsLink, /\/nl\/alle-producten\/?$/i);
    }
  }

  if (!/\/nl\/alle-producten\/?$/i.test(page.url())) {
    await safeGoto(page, new URL(ALL_PRODUCTS_PATH, 'https://vasco-translator.be').href);
  }

  await expect(page).toHaveURL(/\/nl\/alle-producten\/?$/i);
...
async function addProductToCart(page, product, expectedCartCount) {
  ...
  if (!(await productCard.isVisible().catch(() => false))) {
    await safeGoto(page, new URL(ALL_PRODUCTS_PATH, BASE_URL).href).catch(() => {});
    await closeBlockingPopups(page);
    await closeCartDialog(page);
    await closeMenuOverlay(page);
  }
...
async function expectCartCount(page, expectedCartCount) {
  const cartLinkByName = page.getByRole('link', { name: new RegExp(`Winkelwagen\\s*${expectedCartCount}`, 'i') }).first();
...
async function closeCartDialog(page) {
  ...
  const closeButton = blockCartDialog.getByRole('button', { name: /Sluiten|Close/i }).first();
...
async function closeBlockingPopups(page) {
  const popupCloseButtons = [
    ...
    page.locator('button[aria-label="Sluiten"], button[aria-label="Close"]').first(),
  ];
...
async function findVisibleContinueButton(page) {
  ...
  const buttons = page.getByRole('button', { name: /Verder|Doorgaan/i });
...
async function isAddressFormReady(page) {
  if (await hasVisibleText(page, /Wat is uw factuuradres|Wat is je factuuradres|factuuradres/i)) return true;

  return (
    (await hasVisibleFieldForPattern(page, /^enter adres$|^invoeren adres$|^adres$/i)) ||
    (await hasVisibleFieldForPattern(page, /postcode/i))
  );
}

async function chooseIndividualCustomerType(page) {
  const individualTargets = [
    page.getByRole('radio', { name: /Particulier|Privéklant|Individuele klant/i }).first(),
    page.getByLabel(/Particulier|Privéklant|Individuele klant/i).first(),
    page.locator('label').filter({ hasText: /Particulier|Privéklant|Individuele klant/i }).first(),
    page.getByText(/Particulier|Privéklant|Individuele klant/i).first(),
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    try {
      ...
      if (/403 Forbidden|Unable to connect/i.test(bodyText)) {
        throw new Error(`Store unavailable for ${url}`);
      }
      return;
    } catch (error) {
      lastError = error;
...
async function fillPhoneNumber(page, value) {
  const candidates = [
    page.getByRole('textbox', { name: /Telefoon/i }).last(),
    page.getByLabel(/Telefoon/i).last(),
    page.locator('input[type="tel"]').last(),
    page.locator('input[name*="phone" i], input[id*="phone" i], input[name*="telephone" i], input[id*="telephone" i], input[name*="telefoon" i], input[id*="telefoon" i]').last(),
  ];
