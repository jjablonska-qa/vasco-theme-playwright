...
const BASE_URL = 'https://vasco-electronics.cz/';
const ALL_PRODUCTS_PATH = '/vsechny-vyrobky/';
const CHECKOUT_PATH = '/objednavka';
const CART_PATH = '/kosik?action=show';
const PRODUCT_Q1 = { id: '38' };

const SHIPPING_METHOD_LABEL = 'Česká pošta - doručení na adresu';
const COD_PAYMENT_LABEL = 'Platba na dobírku';
...
for (const scenario of scenarios) {
  test(`CZ COD dla ${scenario.quantity}x Q1 jest ${scenario.expectedCodVisible ? 'widoczne' : 'ukryte'} - ${scenario.label}`, async ({ page, browserName }) => {
    await seedCartWithQ1Quantity(page, scenario.quantity);
    await goToCheckout(page);
    await completePersonalInformation(page, browserName);
    await completeAddress(page, CZ_CUSTOMER);
...
async function goToCheckout(page) {
  ...
  await expect(page.getByText(/Osobni udaje|Osobní údaje|Osobni informace|Osobní informace/i).first()).toBeVisible({ timeout: 20000 });
}

async function completePersonalInformation(page, browserName) {
  await fillField(page, [/^Jmeno|^Jméno|^Krestni jmeno|^Křestní jméno/i], CZ_CUSTOMER.firstName);
  await fillField(page, [/^Prijmeni|^Příjmení/i], CZ_CUSTOMER.lastName);
  await fillField(page, [/E-mail/i], createUniqueEmail(CZ_CUSTOMER.email));
...
async function completeAddress(page, customer) {
  ...
  await fillByPlaceholder(page, /jmeno|jméno|krestni jmeno|křestní jméno/i, customer.firstName);
  await fillByPlaceholder(page, /prijmeni|příjmení/i, customer.lastName);
  await fillByPlaceholder(page, /ulice|adresa/i, customer.address1);
  await fillByPlaceholder(page, /cislo popisne|číslo popisné/i, customer.address2);
  await fillByPlaceholder(page, /psc|psč|postovni smerovaci cislo|poštovní směrovací číslo/i, customer.postcode);
  await fillByPlaceholder(page, /mesto\s*\/\s*obec|město\s*\/\s*obec|mesto|město/i, customer.city);
  await fillPhoneNumber(page, customer.phone);

  const continueButton = page.getByRole('button', { name: /Pokracovat|Pokračovat/i }).last();
  await expect(continueButton).toBeVisible({ timeout: 15000 });
...
async function waitForShippingAndPaymentStep(page) {
  await expect
    .poll(async () => {
      await closeBlockingPopups(page);
      const loadingVisible = await hasVisibleText(page, /Prosim, cekejte|Prosím, čekejte|overujeme vase udaje|ověřujeme vaše údaje|kontrolujeme vaše údaje/i);
      if (loadingVisible) {
        return false;
      }

      const shippingReady = await hasVisibleText(page, /Vyberte zpusob doruceni|Vyberte způsob doručení|Vyberte zpusob dopravy|Vyberte způsob dopravy/i);
      const paymentReady = await hasVisibleText(page, /Zvolte zpusob platby|Zvolte způsob platby|Jak byste chteli zaplatit|Jak byste chtěli zaplatit|Jak chcete zaplatit/i);
...
async function selectShippingMethod(page, shippingLabel) {
  ...
  const targetRadio = page.getByRole('radio', { name: shippingPattern }).first();

  await expect(targetRadio).toBeVisible({ timeout: 15000 });

  const candidates = [
    targetRadio,
    page.getByLabel(shippingPattern).first(),
    page.locator('label').filter({ hasText: shippingPattern }).first(),
    page.getByText(shippingPattern).first(),
  ];
...
async function continueToPaymentStep(page) {
  ...
  await expect
    .poll(async () => {
      ...
      return /Zvolte zpusob platby|Zvolte způsob platby|Jak byste chteli zaplatit|Jak byste chtěli zaplatit|Jak chcete zaplatit/i.test(bodyText);
    }, { timeout: 30000 })
    .toBeTruthy();
}
...
async function openAllProducts(page) {
  ...
  const mainMenuLink = page.getByRole('link', { name: /Všechny výrobky|Vsechny vyrobky/i }).first();
  const visibleAllProductsLink = page.locator(`a[href*="${ALL_PRODUCTS_PATH}"]`).first();
  const shopMenuItem = page.getByRole('menuitem', { name: /obchod|překladače|prekladace|výrobky|vyrobky/i }).first();

  if (await desktopMenuLink.isVisible().catch(() => false)) {
    await clickAndWaitForUrl(page, desktopMenuLink, /\/vsechny-vyrobky\/?$/i);
  } else {
    ...
    if (await mainMenuLink.isVisible().catch(() => false)) {
      await clickAndWaitForUrl(page, mainMenuLink, /\/vsechny-vyrobky\/?$/i);
    }

    if (!/\/vsechny-vyrobky\/?$/i.test(page.url()) && (await visibleAllProductsLink.isVisible().catch(() => false))) {
      await clickAndWaitForUrl(page, visibleAllProductsLink, /\/vsechny-vyrobky\/?$/i);
    }

    if (!/\/vsechny-vyrobky\/?$/i.test(page.url())) {
      await Promise.all([
        page.waitForURL(/\/vsechny-vyrobky\/?$/i, { timeout: 15000 }),
        page
          .evaluate(() => {
            const target = Array.from(document.querySelectorAll('a')).find(link => {
              ...
              return href.includes('/vsechny-vyrobky/') && visible;
            });
  ...
  await expect(page).toHaveURL(/\/vsechny-vyrobky\/?$/i);
...
async function closeCartDialog(page) {
  ...
  const closeButton = blockCartDialog.getByRole('button', { name: /Zavrit|Zavřít|Close/i }).first();
...
async function closeBlockingPopups(page) {
  ...
  for (const popupSelector of popupSelectors) {
    ...
    if (await popup.isVisible().catch(() => false)) {
      const saleText = popup.getByText(/sale|sleva|akce/i).first();
...
async function findVisibleContinueButton(page) {
  ...
  const buttons = page.getByRole('button', { name: /Pokracovat|Pokračovat/i });
...
async function clickContinueButton(button) {
  ...
  await button.click({ timeout: 11000 }).catch(async () => {
...
async function isAddressFormReady(page) {
  if (await hasVisibleText(page, /Jaka je vase fakturacni adresa\?|Jaká je vaše fakturační adresa\?|Tyto udaje budou pouzity pro platbu|Tyto údaje budou použity pro platbu/i)) {
    return true;
  }

  return (
    (await hasVisibleFieldForPattern(page, /ulice|adresa/i)) ||
    (await hasVisibleFieldForPattern(page, /psc|psč|postovni smerovaci cislo|poštovní směrovací číslo/i))
  );
}

async function chooseIndividualCustomerType(page) {
  const individualTargets = [
    page.getByRole('radio', { name: /Soukroma osoba|Soukromá osoba|Individualni zakaznik|Individuální zákazník/i }).first(),
    page.getByLabel(/Soukroma osoba|Soukromá osoba|Individualni zakaznik|Individuální zákazník/i).first(),
    page.locator('label').filter({ hasText: /Soukroma osoba|Soukromá osoba|Individualni zakaznik|Individuální zákazník/i }).first(),
    page.getByText(/Soukroma osoba|Soukromá osoba|Individualni zakaznik|Individuální zákazník/i).first(),
...
function createUniqueEmail(email) {
  ...
  const suffix = `czcod${Date.now().toString(36)}`;
