...
const BASE_URL = 'https://traductor-de-voz.es/';
const ALL_PRODUCTS_PATH = '/productos/';
const CHECKOUT_PATH = '/pedido';
const CART_PATH = '/carrito?action=show';
const PRODUCT_Q1 = { id: '38' };

const SHIPPING_METHOD_LABEL = 'CTT Express ES COD (contra reembolso)';
const COD_PAYMENT_LABEL = 'Pago contra reembolso';
...
for (const scenario of scenarios) {
  test(`ES COD dla ${scenario.quantity}x Q1 jest ${scenario.expectedCodVisible ? 'widoczne' : 'ukryte'} - ${scenario.label}`, async ({ page, browserName }) => {
    ...
    await completeAddress(page, ES_CUSTOMER);
...
async function goToCheckout(page) {
  ...
  if (!/\/pedido/i.test(page.url())) {
    await safeGoto(page, checkoutUrl);
  }
  await dismissCookieBanner(page);
  await closeBlockingPopups(page);

  await expect(page).toHaveURL(/\/pedido/i);
  await expect(page.getByText(/Datos Personales|Informacion personal|Información personal|Tus datos/i).first()).toBeVisible({ timeout: 20000 });
}

async function completePersonalInformation(page, browserName) {
  await fillField(page, [/^Nombre/i], ES_CUSTOMER.firstName);
  await fillField(page, [/^Apellido|^Apellidos/i], ES_CUSTOMER.lastName);
  await fillField(page, [/Dirección de correo electrónico|Correo electrónico|Email|E-mail/i], createUniqueEmail(ES_CUSTOMER.email));

  for (const label of [/Términos de Servicio|Términos y condiciones|Condiciones generales|condiciones/i, /Política de Privacidad|Política de privacidad|privacidad/i]) {
...
async function continueToAddressStep(page, browserName) {
  ...
  const addressStepTargets = [
    page.getByText(/^Direcciones$/i).first(),
    page.locator('[role="tab"]').filter({ hasText: /^Direcciones$/i }).first(),
    page.locator('[role="button"]').filter({ hasText: /^Direcciones$/i }).first(),
  ];
...
async function completeAddress(page, customer) {
  ...
  await fillByPlaceholder(page, /nombre/i, customer.firstName);
  await fillByPlaceholder(page, /apellido|apellidos/i, customer.lastName);
  await fillByPlaceholder(page, /^escribe dirección$|^direccion$|^dirección$/i, customer.address1);
  await fillByPlaceholder(page, /dirección complementaria|direccion complementaria|complementaria|complemento|dirección 2|direccion 2|adicional/i, customer.address2).catch(() => {});
  await fillByPlaceholder(page, /código postal|codigo postal|cp/i, customer.postcode);
  await fillByPlaceholder(page, /ciudad|localidad/i, customer.city);
...
async function waitForShippingAndPaymentStep(page) {
  await expect
    .poll(async () => {
      await closeBlockingPopups(page);
      const loadingVisible = await hasVisibleText(page, /Estamos verificando tus datos|Verificando tus datos/i);
      if (loadingVisible) {
        return false;
      }

      const shippingReady = await hasVisibleText(page, /Elige el método de envío|Elige el metodo de envio|Si deseas añadir un comentario sobre tu pedido|Si deseas anadir un comentario sobre tu pedido/i);
      const paymentReady = await hasVisibleText(page, /¿Cómo te gustaría pagar\?|¿Como te gustaria pagar\?|Estos datos se utilizarán para el pago|Estos datos se utilizaran para el pago/i);
...
async function continueToPaymentStep(page) {
  ...
  await expect
    .poll(async () => {
      const bodyText = await page.locator('body').innerText().catch(() => '');
      return /¿Cómo te gustaría pagar\?|¿Como te gustaria pagar\?|Estos datos se utilizarán para el pago|Estos datos se utilizaran para el pago/i.test(bodyText);
    }, { timeout: 30000 })
    .toBeTruthy();
}
...
async function openAllProducts(page) {
  ...
  const mainMenuLink = page.getByRole('link', { name: /Productos/i }).first();
  const visibleAllProductsLink = page.locator(`a[href*="${ALL_PRODUCTS_PATH}"]`).first();
  const shopMenuItem = page.getByRole('menuitem', { name: /tienda|traductor|productos/i }).first();

  if (await desktopMenuLink.isVisible().catch(() => false)) {
    await clickAndWaitForUrl(page, desktopMenuLink, /\/productos\/?$/i);
  } else {
    ...
    if (await mainMenuLink.isVisible().catch(() => false)) {
      await clickAndWaitForUrl(page, mainMenuLink, /\/productos\/?$/i);
    }

    if (!/\/productos\/?$/i.test(page.url()) && (await visibleAllProductsLink.isVisible().catch(() => false))) {
      await clickAndWaitForUrl(page, visibleAllProductsLink, /\/productos\/?$/i);
    }

    if (!/\/productos\/?$/i.test(page.url())) {
      await Promise.all([
        page.waitForURL(/\/productos\/?$/i, { timeout: 15000 }),
        page
          .evaluate(() => {
            const target = Array.from(document.querySelectorAll('a')).find(link => {
              ...
              return href.includes('/productos/') && visible;
            });
...
async function expectCartCount(page, expectedCartCount) {
  const cartLinkByName = page
    .getByRole('link', { name: new RegExp(`Cesta de la compra\\s*${expectedCartCount}|Carrito\\s*${expectedCartCount}|Cesta\\s*${expectedCartCount}`, 'i') })
    .first();
...
async function closeCartDialog(page) {
  ...
  const closeButton = blockCartDialog.getByRole('button', { name: /Cerrar|Close/i }).first();
...
async function closeBlockingPopups(page) {
  ...
  for (const popupSelector of popupSelectors) {
    ...
    if (await popup.isVisible().catch(() => false)) {
      const saleText = popup.getByText(/sale|oferta|descuento|newsletter|suscr/i).first();
...
async function isAddressFormReady(page) {
  if (await hasVisibleText(page, /dirección de facturación|direccion de facturacion|¿Cuál es su dirección de facturación\?|¿Cuál es tu dirección de facturación\?|¿Cual es su direccion de facturacion\?|¿Cual es tu direccion de facturacion\?|Elige si deseas la factura como Particular o Empresa\/Autónomo|Elige si deseas la factura como Particular o Empresa\/Autonomo/i)) {
    return true;
  }

  return (
    (await hasVisibleFieldForPattern(page, /^escribe dirección$|^direccion$|^dirección$/i)) ||
    (await hasVisibleFieldForPattern(page, /código postal|codigo postal|cp/i))
  );
}

async function chooseIndividualCustomerType(page) {
  const individualTargets = [
    page.getByRole('radio', { name: /^Persona$|^Cliente individual$|^Particular$/i }).first(),
    page.getByLabel(/^Persona$|^Cliente individual$|^Particular$/i).first(),
    page.locator('label').filter({ hasText: /^Persona$|^Cliente individual$|^Particular$/i }).first(),
    page.getByText(/^Persona$|^Cliente individual$|^Particular$/i).first(),
...
async function fillPhoneNumber(page, value) {
  const candidates = [
    page.getByRole('textbox', { name: /Teléfono|Telefono/i }).last(),
    page.getByLabel(/Teléfono|Telefono/i).last(),
    page.locator('input[type="tel"]').last(),
    page.locator('input[name*="phone" i], input[id*="phone" i], input[name*="telephone" i], input[id*="telephone" i], input[name*="telefono" i], input[id*="telefono" i]').last(),
  ];
