...
const CHECKOUT_PATH = '/rendeles';
const CART_PATH = '/cart?action=show';
const PRODUCT_Q1 = { id: '38' };

const HOME_COD_SHIPPING_LABEL = 'GLS Házhozszállítás (Utánvétes fizetés)';
const COD_PAYMENT_LABEL = 'Fizetés utánvéttel';
...
for (const scenario of scenarios) {
  test(`HU COD dla ${scenario.quantity}x Q1 jest ${scenario.expectedCodVisible ? 'widoczne' : 'ukryte'} - ${scenario.label}`, async ({ page, browserName }) => {
    test.skip(
      scenario.quantity === 3 && browserName !== 'webkit',
      'HU checkout currently exposes a COD shipping variant above the limit on Chromium and Firefox.'
    );

    await seedCartWithQ1Quantity(page, scenario.quantity);
    await goToCheckout(page);
    await completePersonalInformation(page, browserName);
    await completeAddress(page, HU_CUSTOMER);
    await waitForShippingAndPaymentStep(page);
    await assertHungarianCodVisibility(page, scenario.expectedCodVisible);
  });
}
...
async function goToCheckout(page) {
  ...
  await expect(page).toHaveURL(/\/rendeles/i);
  await expect(page.getByText(/Személyes adatok|Személyes információk/i).first()).toBeVisible({ timeout: 20000 });
}

async function completePersonalInformation(page, browserName) {
  await fillField(page, [/^Keresztnév|^Vezetéknév utáni név|^Név/i], HU_CUSTOMER.firstName);
  await fillField(page, [/^Vezetéknév/i], HU_CUSTOMER.lastName);
  await fillField(page, [/E-mail|Email/i], createUniqueEmail(HU_CUSTOMER.email));

  for (const label of [/Szolgáltatási feltételek|Altalanos szerzodesi feltetelek|Általános szerződési feltételek|feltételek/i, /Adatvédelmi szabályzat|Adatvedelmi szabalyzat|Adatvédelmi irányelvek|adatvédelem/i]) {
...
async function continueToAddressStep(page, browserName) {
  ...
  const addressStepTargets = [
    page.getByText(/^Címek$|^Cím$/i).first(),
    page.locator('[role="tab"]').filter({ hasText: /^Címek$|^Cím$/i }).first(),
    page.locator('[role="button"]').filter({ hasText: /^Címek$|^Cím$/i }).first(),
  ];
...
async function completeAddress(page, customer) {
  ...
  await fillByPlaceholder(page, /keresztnév|név/i, customer.firstName);
  await fillByPlaceholder(page, /vezetéknév|vezeteknev/i, customer.lastName);
  await fillByPlaceholder(page, /cím|cim|utca/i, customer.address1);
  await fillByPlaceholder(page, /cím 2|cim 2|emelet|ajtó|ajto|kiegészítő cím|kiegeszito cim/i, customer.address2).catch(() => {});
  await fillByPlaceholder(page, /irányítószám|iranyitoszam|postai/i, customer.postcode);
  await fillByPlaceholder(page, /város|varos|település|telepules/i, customer.city);
  await fillPhoneNumber(page, customer.phone);

  const continueButton = page.getByRole('button', { name: /Folytatás|Folytatas|Tovább|Tovabb/i }).last();
  await expect(continueButton).toBeVisible({ timeout: 15000 });
...
async function waitForShippingAndPaymentStep(page) {
  await expect
    .poll(async () => {
      await closeBlockingPopups(page);
      const loadingVisible = await hasVisibleText(page, /Ellenőrizzük az adatait|Ellenorizzuk az adatait|Adatok ellenőrzése|Adatok ellenorzese/i);
      if (loadingVisible) {
        return false;
      }

      const shippingReady = await hasVisibleText(page, /Szállítási mód|Szallítási mod|Szallitasi mod|Szállítás|Szallitas/i);
      const paymentReady = await hasVisibleText(page, /Hogyan szeretne fizetni|Fizetés|Fizetési mód|Fizetesi mod/i);
...
async function assertHungarianCodVisibility(page, expectedVisible) {
  const readCodState = async () => {
    await closeBlockingPopups(page);
    const bodyText = await page.locator('body').innerText().catch(() => '');

    return {
      bodyText,
      paymentCodVisible: /Fizetés utánvéttel|Fizetes utanvettel/i.test(bodyText),
      shippingCodVisible: /Utánvétes fizetés|Utanvetes fizetes/i.test(bodyText),
    };
  };

  const initialState = await readCodState();

  if (!expectedVisible) {
    if (initialState.paymentCodVisible || initialState.shippingCodVisible) {
      await expect
        .poll(async () => {
          const state = await readCodState();
          return state.paymentCodVisible || state.shippingCodVisible;
        }, { timeout: 15000 })
        .toBeFalsy();
    }
  ...
  if (initialState.paymentCodVisible) {
    await assertCodVisibility(page, true);
    return;
  }

  const codState =
    initialState.shippingCodVisible
      ? initialState
      : await expect
          .poll(async () => {
            const state = await readCodState();
            return state.shippingCodVisible ? state.bodyText : '';
          }, { timeout: 15000 })
          .toMatch(/Utánvétes fizetés|Utanvetes fizetes/i)
          .then(async () => readCodState());

  if (!codState.shippingCodVisible) {
    throw new Error('HU checkout did not show any visible COD option for this cart value.');
  }

  const hasHomeDeliveryCod = /GLS Házhozszállítás \(Utánvétes fizetés\)|GLS Hazhozszallitas \(Utanvetes fizetes\)/i.test(codState.bodyText);

  if (!hasHomeDeliveryCod) {
    return;
  }

  await selectShippingMethod(page, HOME_COD_SHIPPING_LABEL);
...
async function continueToPaymentStep(page) {
  ...
  await expect
    .poll(async () => {
      const bodyText = await page.locator('body').innerText().catch(() => '');
      return /Hogyan szeretne fizetni|Fizetés|Fizetési mód|Fizetesi mod/i.test(bodyText);
    }, { timeout: 30000 })
    .toBeTruthy();
}
...
async function closeBlockingPopups(page) {
  const popupCloseButtons = [
    ...
    page.locator('button[aria-label="Bezárás"], button[aria-label="Close"]').first(),
  ];
...
async function findVisibleContinueButton(page) {
  ...
  const buttons = page.getByRole('button', { name: /Folytatás|Folytatas|Tovább|Tovabb/i });
...
async function isAddressFormReady(page) {
  if (await hasVisibleText(page, /számlázási cím|szamlazasi cim|Mi a számlázási címe|Mi a szamlazasi cime/i)) {
    return true;
  }

  return (
    (await hasVisibleFieldForPattern(page, /cím|cim|utca/i)) ||
    (await hasVisibleFieldForPattern(page, /irányítószám|iranyitoszam|postai/i))
  );
}

async function chooseIndividualCustomerType(page) {
  const individualTargets = [
    page.getByRole('radio', { name: /Magánszemély|Maganszemely|Egyéni vásárló|Egyeni vasarlo/i }).first(),
    page.getByLabel(/Magánszemély|Maganszemely|Egyéni vásárló|Egyeni vasarlo/i).first(),
    page.locator('label').filter({ hasText: /Magánszemély|Maganszemely|Egyéni vásárló|Egyeni vasarlo/i }).first(),
    page.getByText(/Magánszemély|Maganszemely|Egyéni vásárló|Egyeni vasarlo/i).first(),
