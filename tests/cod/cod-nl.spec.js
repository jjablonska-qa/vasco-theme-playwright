...
const BASE_URL = 'https://vasco-electronics.nl/';
const ALL_PRODUCTS_PATH = '/alle-producten/';
const CHECKOUT_PATH = '/bestelling';
const CART_PATH = '/winkelmandje?action=show';
const PRODUCT_Q1 = { id: '38' };

const SHIPPING_METHOD_LABEL = 'UPS Standaard (onder rembours)';
...
for (const scenario of scenarios) {
  test(`NL COD dla ${scenario.quantity}x Q1 jest ${scenario.expectedCodVisible ? 'widoczne' : 'ukryte'} - ${scenario.label}`, async ({ page, browserName }) => {
    await seedCartWithQ1Quantity(page, scenario.quantity);
    await goToCheckout(page);
    await completePersonalInformation(page, browserName);
    await completeAddress(page, NL_CUSTOMER);
...
async function completePersonalInformation(page, browserName) {
  await fillField(page, [/^Voornaam/i], NL_CUSTOMER.firstName);
  await fillField(page, [/^Achternaam|^Naam/i], NL_CUSTOMER.lastName);
  await fillField(page, [/E-mail|Email/i], createUniqueEmail(NL_CUSTOMER.email));
...
async function completeAddress(page, customer) {
  ...
  await fillByPlaceholder(page, /^invoeren adres$|^adres$/i, customer.address1);
  ...
  const isShippingOrPaymentReady = async () => {
    ...
    return /Verzendingswijze|Verzendmethode|Verzending|Hoe wil je betalen\?|Hoe wilt u betalen\?|Betalingsmethode/i.test(bodyText);
  };
...
async function openAllProducts(page) {
  ...
  const shopMenuItem = page.getByRole('menuitem', { name: /winkel|shop|vertaler|vertalers|producten/i }).first();

  if (await desktopMenuLink.isVisible().catch(() => false)) {
    await clickAndWaitForUrl(page, desktopMenuLink, /\/alle-producten\/?$/i);
  } else {
    ...
    if (await mainMenuLink.isVisible().catch(() => false)) {
      await clickAndWaitForUrl(page, mainMenuLink, /\/alle-producten\/?$/i);
    }

    if (!/\/alle-producten\/?$/i.test(page.url()) && (await visibleAllProductsLink.isVisible().catch(() => false))) {
      await clickAndWaitForUrl(page, visibleAllProductsLink, /\/alle-producten\/?$/i);
    }

    if (!/\/alle-producten\/?$/i.test(page.url())) {
      await Promise.all([
        page.waitForURL(/\/alle-producten\/?$/i, { timeout: 15000 }),
        page
          .evaluate(() => {
            const target = Array.from(document.querySelectorAll('a')).find(link => {
              ...
              return href.includes('/alle-producten/') && visible;
            });
  ...
  await expect(page).toHaveURL(/\/alle-producten\/?$/i);
...
async function isAddressFormReady(page) {
  if (await hasVisibleText(page, /Wat is uw factuuradres|Wat is jouw factuuradres|factuuradres/i)) {
    return true;
  }

  return (
    (await hasVisibleFieldForPattern(page, /^invoeren adres$|^adres$/i)) ||
