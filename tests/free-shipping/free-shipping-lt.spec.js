...
const lt = {
  base: 'https://vasco-translator.lt/',
...
const scenarios = [
  { name: 'poniżej progu', product: lt.glass, free: false },
  { name: 'powyżej progu', product: lt.q1, free: true },
];

for (const scenario of scenarios) {
  test(`LT: darmowa dostawa ${scenario.name}`, async ({ page }) => {
    await page.goto(new URL(lt.products, lt.base).href, { waitUntil: 'domcontentloaded' });
    await acceptCookies(page);
    await addProduct(page, scenario.product);

    await page.goto(new URL(lt.checkout, lt.base).href, { waitUntil: 'domcontentloaded' });
    await acceptCookies(page);
    await fillVisibleField(page, /vardas/i, lt.customer.firstName);
    await fillVisibleField(page, /pavardė|pavarde/i, lt.customer.lastName);
    await fillVisibleField(page, /e-mail|el\.?\s*paštas|email/i, uniqueEmail('lt'));
    await acceptRequiredAgreements(page, [/sąlyg|salyg|naudojimo/i, /privatumo/i]);

    const continueButton = page.getByRole('button', { name: /tęsti|testi/i });
    await continueButton.first().click({ force: true });
    await expect.poll(() => hasVisibleField(page, /adresas|gatvė|gatve/i), { timeout: 30000 }).toBeTruthy();

    await fillVisibleField(page, /vardas/i, lt.customer.firstName);
    await fillVisibleField(page, /pavardė|pavarde/i, lt.customer.lastName);
    await fillVisibleField(page, /^įvesti adresas\*?$|^adresas\*?$|gatvė|gatve/i, lt.customer.address);
    await fillVisibleField(page, /pašto kodas|pasto kodas|kodas/i, lt.customer.postcode);
    await fillVisibleField(page, /miestas|vietovė|vietove/i, lt.customer.city);
    await fillVisibleField(page, /telefonas|telefono/i, lt.customer.phone);
    await continueButton.last().click({ force: true });

    await expect.poll(async () => /pristatymo būdas|pristatymo budas|pristatymas/i.test(await page.locator('body').innerText()), { timeout: 30000 }).toBeTruthy();
    const shipping = await readShippingSummary(page, /^Siuntimas$/i);
    expect(shipping, 'LT shipping summary').not.toBeNull();
    const price = shippingPriceFromText(shipping, { currency: /€|EUR/, freePattern: /nemokam|free/i });
    expect(price, `LT shipping summary: ${shipping}`).not.toBeNull();
...
async function acceptCookies(page) {
  const buttons = [
    ...
    page.getByRole('button', { name: /allow all|accept all|leisti visus/i }).first(),
  ];
...
async function fillVisibleField(page, pattern, value) {
  ...
  throw new Error(`LT field not found: ${pattern}`);
}
