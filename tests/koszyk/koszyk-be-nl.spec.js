...
test('Koszyk BE NL zachowuje dodane produkty po przejsciu do koszyka', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/nl\/winkelwagen/i);
...
async function openCart(page) {
  ...
  if (await blockCartDialog.isVisible().catch(() => false)) {
    const goToCartLink = blockCartDialog.getByRole('link', { name: /Ga naar je winkelwagen/i }).first();
    if (await goToCartLink.isVisible().catch(() => false)) {
      await Promise.all([
        page.waitForURL(/\/nl\/winkelwagen/i, { timeout: 15000 }),
        goToCartLink.click({ force: true }),
      ]).catch(() => {});
    }
  }

  if (/\/nl\/winkelwagen/i.test(page.url())) return;
  ...
  const goToCartLink = page.getByRole('link', { name: /Ga naar je winkelwagen/i }).first();
  if (await goToCartLink.isVisible().catch(() => false)) {
    await Promise.all([
      page.waitForURL(/\/nl\/winkelwagen/i, { timeout: 15000 }),
      goToCartLink.click({ force: true }),
  ...
  if (!/\/nl\/winkelwagen/i.test(page.url())) {
    await page.goto(new URL('/nl/winkelwagen', 'https://vasco-translator.be').href, { waitUntil: 'domcontentloaded' });
  }
}
