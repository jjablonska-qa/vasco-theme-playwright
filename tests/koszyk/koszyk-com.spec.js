...
const BASE_URL = 'https://vasco-translator.com/';
const ALL_PRODUCTS_PATH = '/all-products/';
const CART_PATH = '/cart?action=show';
const PRODUCT_Q1 = { id: '38' };
const PRODUCT_GLASS = { id: '40' };

test('Koszyk COM zachowuje dodane produkty po przejsciu do koszyka', async ({ page }) => {
...
function cartDialog(page) {
  return page
    .locator(
      '#blockcart-modal, [role="dialog"][aria-labelledby="blockcart-modal-title"], [role="dialog"]:has-text("Product added to your shopping cart")'
    )
    .first();
}
...
async function openAllProducts(page) {
  ...
  const mainMenuLink = page.getByRole('link', { name: /All products/i }).first();
  const visibleAllProductsLink = page.locator(`a[href*="${ALL_PRODUCTS_PATH}"]`).first();

  if (await desktopMenuLink.isVisible().catch(() => false)) {
    await clickAndWaitForUrl(page, desktopMenuLink, /\/all-products\/?$/i, 8000);
  } else {
    if (await mainMenuLink.isVisible().catch(() => false)) {
      await clickAndWaitForUrl(page, mainMenuLink, /\/all-products\/?$/i, 8000);
    }

    if (!/\/all-products\/?$/i.test(page.url()) && (await visibleAllProductsLink.isVisible().catch(() => false))) {
      await clickAndWaitForUrl(page, visibleAllProductsLink, /\/all-products\/?$/i, 8000);
    }
  }

  if (!/\/all-products\/?$/i.test(page.url())) {
    await page.goto(new URL(ALL_PRODUCTS_PATH, BASE_URL).href, { waitUntil: 'domcontentloaded' });
  }

  await expect(page).toHaveURL(/\/all-products\/?$/i);
  await closeMenuOverlay(page);
}

async function clickAndWaitForUrl(page, link, urlPattern, timeout = 15000) {
  try {
    await Promise.all([
...
async function addProductToCart(page, product, expectedCartCount) {
  const productCard = page
    .locator(`article.product-miniature[data-id-product="${product.id}"]`)
    .filter({ has: page.locator('button.add-to-cart:not([disabled])') })
    .first();
  const addToCartButton = productCard.locator('button.add-to-cart:not([disabled])').first();
  await closeBlockingPopups(page);
  ...
  await addToCartButton.click({ force: true }).catch(async () => {
    await addToCartButton.evaluate(element => element.click()).catch(() => {});
  });
...
async function expectCartCount(page, expectedCartCount) {
  const cartLinkByName = page
    .getByRole('link', { name: new RegExp(`Shopping\\s*cart\\s*${expectedCartCount}|Cart\\s*${expectedCartCount}`, 'i') })
    .first();
  if (await cartLinkByName.isVisible().catch(() => false)) {
    await expect(cartLinkByName).toBeVisible({ timeout: 15000 });
    return;
  }
  await expect(headerCartLink(page).locator('.cart-count')).toContainText(String(expectedCartCount), { timeout: 15000 });
}
...
async function closeCartDialog(page) {
  ...
  const closeButton = blockCartDialog.getByRole('button', { name: /Close/i }).first();
...
async function closeBlockingPopups(page) {
  const popupCloseButtons = [
    ...
    page.getByRole('button', { name: /Zgadzam się|Zgadzam sie/i }).first(),
  ];
