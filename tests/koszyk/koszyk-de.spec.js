...
test('Koszyk DE zachowuje dodane produkty po przejsciu do koszyka', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/warenkorb/i);
...
async function openCart(page) {
  ...
  if (await blockCartDialog.isVisible().catch(() => false)) {
    ...
    if (await goToCartLink.isVisible().catch(() => false)) {
      await Promise.all([
        page.waitForURL(/\/warenkorb/i, { timeout: 15000 }),
        goToCartLink.click({ force: true }),
      ]).catch(() => {});
    }
  }

  if (/\/warenkorb/i.test(page.url())) return;
  ...
  if (!/\/warenkorb/i.test(page.url())) {
    await page.goto(new URL('/warenkorb', BASE_URL).href, { waitUntil: 'domcontentloaded' });
  }
}

async function expectCartCount(page, expectedCartCount) {
  const cartLinkByName = page.getByRole('link', { name: new RegExp(`Warenkorb\\s*${expectedCartCount}`, 'i') }).first();
...
async function closeBlockingPopups(page) {
  const popupCloseButtons = [
    ...
    page.locator('button[aria-label="Schließen"], button[aria-label="Schliessen"], button[aria-label="Close"]').first(),
  ];
