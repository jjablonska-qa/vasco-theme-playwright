...
test('Koszyk ES zachowuje dodane produkty po przejsciu do koszyka', async ({ page }) => {
...
async function openCart(page) {
  ...
  if (await blockCartDialog.isVisible().catch(() => false)) {
    ...
    if (await goToCartLink.isVisible().catch(() => false)) {
      await Promise.all([
        page.waitForURL(/\/carrito/i, { timeout: 15000 }),
        goToCartLink.click({ force: true }),
      ]).catch(() => {});
    }
  }

  if (/\/carrito/i.test(page.url())) {
    return;
  }
  ...
  if (!/\/carrito/i.test(page.url())) {
    await page.goto(new URL('/carrito', BASE_URL).href, { waitUntil: 'domcontentloaded' });
  }
}

async function expectCartCount(page, expectedCartCount) {
  const cartLinkByName = page
    .getByRole('link', { name: new RegExp(`Carrito\\s*${expectedCartCount}`, 'i') })
    .first();
...
async function closeBlockingPopups(page) {
  const popupCloseButtons = [
    ...
    page.locator('button[aria-label="Cerrar"], button[aria-label="Close"]').first(),
