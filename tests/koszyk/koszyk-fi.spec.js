...
const BASE_URL = 'https://vasco-translator.fi/';
const ALL_PRODUCTS_PATH = '/kaikki-tuotteet/';
const CART_PATH = '/ostoskori?action=show';
const PRODUCT_Q1 = { id: '38' };
const PRODUCT_GLASS = { id: '40' };

test('Koszyk FI zachowuje dodane produkty po przejsciu do koszyka', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/ostoskori/i);
...
async function openAllProducts(page) {
  ...
  const mainMenuLink = page.getByRole('link', { name: /Kaikki tuotteet/i }).first();
  const visibleAllProductsLink = page.locator(`a[href*="${ALL_PRODUCTS_PATH}"]`).first();
  const shopMenuItem = page.getByRole('menuitem', { name: /kauppa|shop|kääntäjät|kaantajat|tuotteet/i }).first();

  if (await desktopMenuLink.isVisible().catch(() => false)) {
    await clickAndWaitForUrl(page, desktopMenuLink, /\/kaikki-tuotteet\/?$/i);
  } else {
    ...
    if (await mainMenuLink.isVisible().catch(() => false)) {
      await clickAndWaitForUrl(page, mainMenuLink, /\/kaikki-tuotteet\/?$/i);
    }

    if (!/\/kaikki-tuotteet\/?$/i.test(page.url()) && (await visibleAllProductsLink.isVisible().catch(() => false))) {
      await clickAndWaitForUrl(page, visibleAllProductsLink, /\/kaikki-tuotteet\/?$/i);
    }

    if (!/\/kaikki-tuotteet\/?$/i.test(page.url())) {
      await Promise.all([
        page.waitForURL(/\/kaikki-tuotteet\/?$/i, { timeout: 15000 }),
        page
          .evaluate(() => {
            const target = Array.from(document.querySelectorAll('a')).find(link => {
              ...
              return href.includes('/kaikki-tuotteet/') && visible;
            });
  ...
  await expect(page).toHaveURL(/\/kaikki-tuotteet\/?$/i);
...
async function openCart(page) {
  ...
  if (await blockCartDialog.isVisible().catch(() => false)) {
    ...
    if (await goToCartLink.isVisible().catch(() => false)) {
      await Promise.all([
        page.waitForURL(/\/ostoskori/i, { timeout: 15000 }),
        goToCartLink.click({ force: true }),
      ]).catch(() => {});
    }
  }

  if (/\/ostoskori/i.test(page.url())) return;
  ...
  if (!/\/ostoskori/i.test(page.url())) {
    await page.goto(new URL('/ostoskori', BASE_URL).href, { waitUntil: 'domcontentloaded' });
  }
}

async function expectCartCount(page, expectedCartCount) {
  const cartLinkByName = page.getByRole('link', { name: new RegExp(`Ostoskori\\s*${expectedCartCount}`, 'i') }).first();
...
async function closeCartDialog(page) {
  ...
  const closeButton = blockCartDialog.getByRole('button', { name: /Sulje|Close/i }).first();
...
async function closeBlockingPopups(page) {
  const popupCloseButtons = [
    ...
    page.locator('button[aria-label="Sulje"], button[aria-label="Close"]').first(),
  ];
