...
const BASE_URL = 'https://vasco-electronics.fr/';
const ALL_PRODUCTS_PATH = '/traducteur-electronique/';
const CART_PATH = '/panier?action=show';
const PRODUCT_Q1 = { id: '38' };
const PRODUCT_GLASS = { id: '40' };

test('Koszyk FR zachowuje dodane produkty po przejsciu do koszyka', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/panier/i);
...
async function openAllProducts(page) {
  ...
  const shopMenuItem = page.getByRole('menuitem', { name: /boutique|traducteurs|produits|shop/i }).first();

  if (await desktopMenuLink.isVisible().catch(() => false)) {
    await clickAndWaitForUrl(page, desktopMenuLink, /\/traducteur-electronique\/?$/i);
  } else {
    ...
    if (await mainMenuLink.isVisible().catch(() => false)) {
      await clickAndWaitForUrl(page, mainMenuLink, /\/traducteur-electronique\/?$/i);
    }

    if (!/\/traducteur-electronique\/?$/i.test(page.url()) && (await visibleAllProductsLink.isVisible().catch(() => false))) {
      await clickAndWaitForUrl(page, visibleAllProductsLink, /\/traducteur-electronique\/?$/i);
    }
  }

  if (!/\/traducteur-electronique\/?$/i.test(page.url())) {
    await page.goto(new URL(ALL_PRODUCTS_PATH, BASE_URL).href, { waitUntil: 'domcontentloaded' });
  }

  await expect(page).toHaveURL(/\/traducteur-electronique\/?$/i);
...
async function openCart(page) {
  ...
  if (await blockCartDialog.isVisible().catch(() => false)) {
    ...
    if (await goToCartLink.isVisible().catch(() => false)) {
      await Promise.all([
        page.waitForURL(/\/panier/i, { timeout: 15000 }),
        goToCartLink.click({ force: true }),
      ]).catch(() => {});
    }
  }

  if (/\/panier/i.test(page.url())) return;
  ...
  if (!/\/panier/i.test(page.url())) {
    await page.goto(new URL('/panier', BASE_URL).href, { waitUntil: 'domcontentloaded' });
  }
}
