...
const BASE_URL = 'https://vasco-translator.hr/';
const ALL_PRODUCTS_PATH = '/proizvodi/';
const CART_PATH = '/kosarica?action=show';
const PRODUCT_Q1 = { id: '38' };
const PRODUCT_GLASS = { id: '40' };

test('Koszyk HR zachowuje dodane produkty po przejsciu do koszyka', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/kosarica/i);
...
async function openAllProducts(page) {
  ...
  const mainMenuLink = page.getByRole('link', { name: /Proizvodi/i }).first();
  const visibleAllProductsLink = page.locator(`a[href*="${ALL_PRODUCTS_PATH}"]`).first();
  const shopMenuItem = page.getByRole('menuitem', { name: /trgovina|prevoditelji|proizvodi/i }).first();

  if (await desktopMenuLink.isVisible().catch(() => false)) {
    await clickAndWaitForUrl(page, desktopMenuLink, /\/proizvodi\/?$/i);
  } else {
    ...
    if (await mainMenuLink.isVisible().catch(() => false)) {
      await clickAndWaitForUrl(page, mainMenuLink, /\/proizvodi\/?$/i);
    }

    if (!/\/proizvodi\/?$/i.test(page.url()) && (await visibleAllProductsLink.isVisible().catch(() => false))) {
      await clickAndWaitForUrl(page, visibleAllProductsLink, /\/proizvodi\/?$/i);
    }

    if (!/\/proizvodi\/?$/i.test(page.url())) {
      await Promise.all([
        page.waitForURL(/\/proizvodi\/?$/i, { timeout: 15000 }),
        page
          .evaluate(() => {
            const target = Array.from(document.querySelectorAll('a')).find(link => {
              ...
              return href.includes('/proizvodi/') && visible;
            });
  ...
  await expect(page).toHaveURL(/\/proizvodi\/?$/i);
...
async function openCart(page) {
  ...
  if (await blockCartDialog.isVisible().catch(() => false)) {
    ...
    if (await goToCartLink.isVisible().catch(() => false)) {
      await Promise.all([
        page.waitForURL(/\/kosarica/i, { timeout: 15000 }),
        goToCartLink.click({ force: true }),
      ]).catch(() => {});
    }
  }

  if (/\/kosarica/i.test(page.url())) return;
  ...
  if (!/\/kosarica/i.test(page.url())) {
    await page.goto(new URL('/kosarica', BASE_URL).href, { waitUntil: 'domcontentloaded' });
  }
}

async function expectCartCount(page, expectedCartCount) {
  const cartLinkByName = page.getByRole('link', { name: new RegExp(`Košarica\\s*${expectedCartCount}|Kosarica\\s*${expectedCartCount}`, 'i') }).first();
...
async function closeCartDialog(page) {
  ...
  const closeButton = blockCartDialog.getByRole('button', { name: /Zatvori|Close/i }).first();
...
async function closeBlockingPopups(page) {
  const popupCloseButtons = [
    ...
    page.locator('button[aria-label="Zatvori"], button[aria-label="Close"]').first(),
