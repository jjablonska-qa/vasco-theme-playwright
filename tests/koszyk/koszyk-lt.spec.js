...
test('Koszyk LT zachowuje dodane produkty po przejsciu do koszyka', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/krepselis/i);
...
async function openAllProducts(page) {
  ...
  const shopMenuItem = page.getByRole('menuitem', { name: /vertimo įrenginiai|produktai|priedai/i }).first();
...
async function openCart(page) {
  ...
  if (await blockCartDialog.isVisible().catch(() => false)) {
    ...
    if (await goToCartLink.isVisible().catch(() => false)) {
      await Promise.all([
        page.waitForURL(/\/krepselis/i, { timeout: 15000 }),
        goToCartLink.click({ force: true }),
      ]).catch(() => {});
    }
  }

  if (/\/krepselis/i.test(page.url())) return;
  ...
  if (!/\/krepselis/i.test(page.url())) {
    await page.goto(new URL('/krepselis', BASE_URL).href, { waitUntil: 'domcontentloaded' });
  }
}
