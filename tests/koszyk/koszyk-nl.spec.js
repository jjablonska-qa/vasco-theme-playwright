...
test('Koszyk NL zachowuje dodane produkty po przejsciu do koszyka', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/winkelmandje/i);
...
async function openCart(page) {
  ...
  if (await blockCartDialog.isVisible().catch(() => false)) {
    ...
    if (await goToCartLink.isVisible().catch(() => false)) {
      await Promise.all([
        page.waitForURL(/\/winkelmandje/i, { timeout: 15000 }),
        goToCartLink.click({ force: true }),
      ]).catch(() => {});
    }
  }

  if (/\/winkelmandje/i.test(page.url())) return;
  ...
  if (!/\/winkelmandje/i.test(page.url())) {
    await page.goto(new URL('/winkelmandje', BASE_URL).href, { waitUntil: 'domcontentloaded' });
  }
}
