...
test('Koszyk PT zachowuje dodane produkty po przejsciu do koszyka', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/carrinho/i);
...
async function openAllProducts(page) {
  ...
  if (await desktopMenuLink.isVisible().catch(() => false)) {
    ...
    if (!/\/todos-produtos\/?$/i.test(page.url())) {
      await Promise.all([
        page.waitForURL(/\/todos-produtos\/?$/i, { timeout: 15000 }),
        page
          .evaluate(() => {
            const target = Array.from(document.querySelectorAll('a')).find(link => {
              ...
              return href.includes('/todos-produtos/') && visible;
            });
...
async function openCart(page) {
  ...
  if (await blockCartDialog.isVisible().catch(() => false)) {
    ...
    if (await goToCartLink.isVisible().catch(() => false)) {
      await Promise.all([
        page.waitForURL(/\/carrinho/i, { timeout: 15000 }),
        goToCartLink.click({ force: true }),
      ]).catch(() => {});
    }
  }

  if (/\/carrinho/i.test(page.url())) return;
  ...
  if (!/\/carrinho/i.test(page.url())) {
    await page.goto(new URL('/carrinho', BASE_URL).href, { waitUntil: 'domcontentloaded' });
  }
}
...
async function closeBlockingPopups(page) {
  const popupCloseButtons = [
    ...
    page.locator('button[aria-label="Fechar"], button[aria-label="Close"]').first(),
  ];
