...
test('Koszyk RO zachowuje dodane produkty po przejsciu do koszyka', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/cos/i);
...
async function openAllProducts(page) {
  ...
  if (await desktopMenuLink.isVisible().catch(() => false)) {
    ...
    if (!/\/toate-produsele\/?$/i.test(page.url())) {
      await Promise.all([
        page.waitForURL(/\/toate-produsele\/?$/i, { timeout: 15000 }),
        page
          .evaluate(() => {
            const target = Array.from(document.querySelectorAll('a')).find(link => {
              ...
              return href.includes('/toate-produsele/') && visible;
            });
...
async function openCart(page) {
  ...
  if (await blockCartDialog.isVisible().catch(() => false)) {
    ...
    if (await goToCartLink.isVisible().catch(() => false)) {
      await Promise.all([
        page.waitForURL(/\/cos/i, { timeout: 15000 }),
        goToCartLink.click({ force: true }),
      ]).catch(() => {});
    }
  }

  if (/\/cos/i.test(page.url())) return;
  ...
  if (!/\/cos/i.test(page.url())) {
    await page.goto(new URL('/cos', BASE_URL).href, { waitUntil: 'domcontentloaded' });
  }
}
