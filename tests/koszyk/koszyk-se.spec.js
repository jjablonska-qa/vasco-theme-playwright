...
const BASE_URL = 'https://vasco-translator.se/';
const ALL_PRODUCTS_PATH = '/alla-produkter/';
const CART_PATH = '/varukorg?action=show';
const PRODUCT_Q1 = { id: '38' };
const PRODUCT_GLASS = { id: '40' };

test('Koszyk SE zachowuje dodane produkty po przejsciu do koszyka', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/varukorg/i);
...
async function openAllProducts(page) {
  ...
  const mainMenuLink = page.getByRole('link', { name: /Alla produkter/i }).first();
  const visibleAllProductsLink = page.locator(`a[href*="${ALL_PRODUCTS_PATH}"]`).first();
  const shopMenuItem = page.getByRole('menuitem', { name: /butik|översättare|oversattare|produkter/i }).first();

  if (await desktopMenuLink.isVisible().catch(() => false)) {
    await clickAndWaitForUrl(page, desktopMenuLink, /\/alla-produkter\/?$/i);
  } else {
    ...
    if (await mainMenuLink.isVisible().catch(() => false)) {
      await clickAndWaitForUrl(page, mainMenuLink, /\/alla-produkter\/?$/i);
    }

    if (!/\/alla-produkter\/?$/i.test(page.url()) && (await visibleAllProductsLink.isVisible().catch(() => false))) {
      await clickAndWaitForUrl(page, visibleAllProductsLink, /\/alla-produkter\/?$/i);
    }

    if (!/\/alla-produkter\/?$/i.test(page.url())) {
      await Promise.all([
        page.waitForURL(/\/alla-produkter\/?$/i, { timeout: 15000 }),
        page
          .evaluate(() => {
            const target = Array.from(document.querySelectorAll('a')).find(link => {
              ...
              return href.includes('/alla-produkter/') && visible;
            });
  ...
  await expect(page).toHaveURL(/\/alla-produkter\/?$/i);
...
async function openCart(page) {
  ...
  if (await blockCartDialog.isVisible().catch(() => false)) {
    ...
    if (await goToCartLink.isVisible().catch(() => false)) {
      await Promise.all([
        page.waitForURL(/\/varukorg/i, { timeout: 15000 }),
        goToCartLink.click({ force: true }),
      ]).catch(() => {});
    }
  }

  if (/\/varukorg/i.test(page.url())) return;
  ...
  if (!/\/varukorg/i.test(page.url())) {
    await page.goto(new URL('/varukorg', BASE_URL).href, { waitUntil: 'domcontentloaded' });
  }
}

async function expectCartCount(page, expectedCartCount) {
  const cartLinkByName = page.getByRole('link', { name: new RegExp(`Varukorg\\s*${expectedCartCount}`, 'i') }).first();
...
async function closeCartDialog(page) {
  ...
  const closeButton = blockCartDialog.getByRole('button', { name: /Stäng|Stang|Close/i }).first();
...
async function closeBlockingPopups(page) {
  const popupCloseButtons = [
    ...
    page.locator('button[aria-label="Stäng"], button[aria-label="Close"]').first(),
