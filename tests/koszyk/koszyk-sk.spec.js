...
test('Koszyk SK zachowuje dodane produkty po przejsciu do koszyka', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/nakupny-kosik/i);
...
async function openAllProducts(page) {
  ...
  const shopMenuItem = page.getByRole('menuitem', { name: /obchod|preklad|produkty/i }).first();
  ...
  if (await desktopMenuLink.isVisible().catch(() => false)) {
    ...
    if (!/\/vsetky-produkty\/?$/i.test(page.url())) {
      await Promise.all([
        page.waitForURL(/\/vsetky-produkty\/?$/i, { timeout: 15000 }),
        page
          .evaluate(() => {
            const target = Array.from(document.querySelectorAll('a')).find(link => {
              ...
              return href.includes('/vsetky-produkty/') && visible;
            });
...
async function openCart(page) {
  ...
  if (await blockCartDialog.isVisible().catch(() => false)) {
    ...
    if (await goToCartLink.isVisible().catch(() => false)) {
      await Promise.all([
        page.waitForURL(/\/nakupny-kosik/i, { timeout: 15000 }),
        goToCartLink.click({ force: true }),
      ]).catch(() => {});
    }
  }

  if (/\/nakupny-kosik/i.test(page.url())) return;
  ...
  if (!/\/nakupny-kosik/i.test(page.url())) {
    await page.goto(new URL('/nakupny-kosik', BASE_URL).href, { waitUntil: 'domcontentloaded' });
  }
}
...
async function closeBlockingPopups(page) {
  const popupCloseButtons = [
    ...
    page.locator('button[aria-label="Zavrieť"], button[aria-label="Zavriet"], button[aria-label="Close"]').first(),
  ];
