...
const BASE_URL = 'https://vasco-electronics.co.uk/';
...
test('Koszyk UK zachowuje dodane produkty po przejsciu do koszyka', async ({ page }) => {
...
async function openAllProducts(page) {
  ...
  const shopMenuItem = page.getByRole('menuitem', { name: /shop|translators|products/i }).first();

  if (await desktopMenuLink.isVisible().catch(() => false)) {
    await clickAndWaitForUrl(page, desktopMenuLink, /\/all-products\/?$/i);
  } else {
    ...
    if (await mainMenuLink.isVisible().catch(() => false)) {
      await clickAndWaitForUrl(page, mainMenuLink, /\/all-products\/?$/i);
    }

    if (!/\/all-products\/?$/i.test(page.url()) && (await visibleAllProductsLink.isVisible().catch(() => false))) {
      await clickAndWaitForUrl(page, visibleAllProductsLink, /\/all-products\/?$/i);
    }

    if (!/\/all-products\/?$/i.test(page.url())) {
      await Promise.all([
        page.waitForURL(/\/all-products\/?$/i, { timeout: 15000 }),
        page
          .evaluate(() => {
            const target = Array.from(document.querySelectorAll('a')).find(link => {
              ...
              return href.includes('/all-products/') && visible;
            });
...
async function openCart(page) {
  ...
  if (await blockCartDialog.isVisible().catch(() => false)) {
    const goToCartLink = blockCartDialog.getByRole('link', { name: /Go to your cart/i }).first();
  ...
  const goToCartLink = page.getByRole('link', { name: /Go to your cart/i }).first();
  if (await goToCartLink.isVisible().catch(() => false)) {
    await Promise.all([
      page.waitForURL(/\/cart/i, { timeout: 15000 }),
      goToCartLink.click({ force: true }),
...
async function expectCartCount(page, expectedCartCount) {
  await expect(
    page.getByRole('link', { name: new RegExp(`Cart\\s*${expectedCartCount}`, 'i') }).first()
